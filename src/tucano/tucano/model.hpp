...
public:
	...
	*/
    ...
    void normalizeModelMatrix (void)
    {
        ...
        model_matrix.scale(normalization_scale);
     ...
     * at the origin, and scale the model to fit inside a unit sphere.
     */
    void normalizeModelMatrix (const Eigen::Vector3f &centroid_, float normalization_scale_)
    {
        model_matrix = Eigen::Affine3f::Identity();

        model_matrix.scale(normalization_scale_);
        model_matrix.translate(-centroid_);
    }
    ...
    * @brief Desnormalize model matrix
    */
    void desnormalizeModelMatrix (const Eigen::Vector3f &centroid_, float normalization_scale_)
    {
        model_matrix.translate(centroid_);
        model_matrix.scale(1.0/normalization_scale_);
    }
