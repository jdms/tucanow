...
//      insertPausePositions();
    ...
    * The camera will pause at this position for given number of steps, default is zero
	* @param camera Camera containing position and orientation in view matrix
    * @param interval Number of steps to stay at this position
	*/
	...
	/**
	* @brief Add an optial key direction
	* Key directions are interpolated via SLERP and not SQUAD
	* This is specially useful for interpolating a light direction together with the camera path
	* Key directions should be always allocated together with a key position to work properly
	* @param camera Camera with directional information
	*/
...
//		expq.vec() = exp(q.w()) * q.vec().normalized() * sin( q.vec().norm() );
	...
	*/
	...
	/**
	* @brief return a direction at a given path parameter
	* @return quaternion with interpolated direction
	*/
	...
	*/
	...
	/**
	* @brief Return the direction at current animation time
	* @return Interpolated direction as Quaternion at current animation time
	*/
	...
	*/
    ...
    void writeToFile (const string filename)
    {
        ...
        for (unsigned int i = 0; i < key_positions.size(); ++i)
        {
			...
			// In case a direction is not provided, the key quaternion is used instead.
			if( i < key_directions.size() )
			{
